...
class mixF(nn.Module):
    def __init__(self, nvals, d_model, patch_num, num_heads=8, window_size=8, head_dropout=0, alpha=0.2, beta=0.5):
        super().__init__()
        ...
        self.d_model = d_model
        #high frenquent
        self.h_attention = MixFAttention(nvals=nvals, patch_num=patch_num, d_model=d_model, window_size=window_size, beta=beta, num_heads=self.h_heads)
        ...
        self.l_attention = nn.Sequential(nn.Conv2d(nvals, nvals, kernel_size=window_size, groups=nvals, padding='same'),
                                      nn.GELU(),
...
class MixFAttention(nn.Module):
    def __init__(self, nvals, patch_num, d_model, window_size=8, beta=0.5, num_heads=8):
        super().__init__()
        ...
        self.l_conv1d = nn.Sequential(
                                    nn.Conv2d(nvals, nvals, kernel_size=window_size, padding='same', groups=nvals),
                                    )
    ...
    def l_attention(self, x):
        ...
        q = self.l_q_linear(q).reshape(B, N, P, self.l_heads, self.l_dim).permute(0, 1, 3, 2, 4) # [batch*features, l_heads, patch_size, d_model/l_heads]
        k = self.l_k_linear(low_x).reshape(B, N, -1, self.l_heads, self.l_dim).permute(0, 1, 3, 2, 4)# [batch*features, l_heads, patch_size/ws, d_model/l_heads]
        v = self.l_v_linear(low_x).reshape(B, N, -1, self.l_heads, self.l_dim).permute(0, 1, 3, 2, 4) # [batch*features, l_heads, patch_size/ws, d_model/l_heads]
        attn = (q @ k.transpose(-2, -1)) * self.l_scale
        attn = attn.softmax(dim=-1)
        attn = (attn @ v).transpose(1, 2).reshape(B, N, -1, dim) # [batch*features, patch_size, d_model]
        return attn
    
    def h_attention(self, x):
        B, N, P, dim = x.shape # [batch*features, patch_size, d_model]
        q = self.h_v_linear(x).reshape(B, N, P, self.h_heads, self.h_dim).permute(0, 1, 3, 2, 4) # [batch*features, h_heads, patch_size, d_model/h_heads]
        k = self.h_v_linear(x).reshape(B, N, P, self.h_heads, self.h_dim).permute(0, 1, 3, 2, 4) # [batch*features, h_heads, patch_size, d_model/h_heads]
        v = self.h_v_linear(x).reshape(B, N, P, self.h_heads, self.h_dim).permute(0, 1, 3, 2, 4) # [batch*features, h_heads, patch_size, d_model/h_heads]
        attn = (q @ k.transpose(-2, -1)) * self.h_scale
        attn = attn.softmax(dim=-1)
        attn = (attn @ v).transpose(1, 2).reshape(B, N, -1, dim)
        return attn
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 调整形状以进行拼接
        scores = scores.transpose(1, 2).contiguous().view(batch, n, -1, self.d_model)
